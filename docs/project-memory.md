...
- **Eval C2 混合 scope 评测**（见已完成模块第 34 条）：真库 15 个能力词——词面 `lexical_precision@k=0.97`、判官 `judged_precision@k=0.99`、`noise@k=0.01`、`semantic_gain@k=0.02`（高频能力词词面已近天花板、语义增量小；语义价值在"能力被描述但不点名"的查询更大）。**环境坑修复**：openai SDK 依赖的 `jiter` 原生扩展 3.12 版经 `.pth` 桥泄漏进 3.11 conda-env、加载失败，被判官 `except` 吞成假 `judged_precision=0.00`；已装 `jiter==0.16.0`(cp311) 修复，并去掉判官的异常吞咽（系统性失败改为响亮报错）。
- **Eval C1 扰动鲁棒性评测**（见已完成模块第 33 条）：真库六类型表——格式类（去后缀/整句/全半角）recovery=1.00；错字类同音字 0.72 / 漏字 0.60 / 相邻字对调 0.56（单点错比对调温和，片段兜底救回多数）；wrong 恒 0（从不误解析别家）。
...
33. **Eval C1 扰动鲁棒性评测**（新 `eval/perturb.py` + `golden_gen`/`metrics`/`runner`/CLI/脚本扩展，分支 `feature/eval-c1-perturbation`）：给企业识别补“真实输入变形”评测，量化 `resolve_supplier` 对用户实际打法的鲁棒性。**来源法真值**（每条扰动从已知企业 X 生成、理想答案恒解析到 X；种子唯一性用**独立粗粒度子串扫描**保证——非 resolver 两段式逻辑，故非循环）。四扰动类型：`drop_suffix`（去后缀）/`transpose`（相邻字对调，错字确定性代理、不引混淆字典）/`width_variant`（ASCII 转全角，NFKC 应吸收）/`noise_wrap`（整句包裹）。`perturbation_metrics` 按类型算 recovery/wrong/miss（`resolve_supplier` 返回=源 X 为回收、别家为误解析、not_found/ambiguous 为漏解析），CLI `eval perturb` 打印按类型表、无真名。起草脚本 stdout 只回条数，真名进 gitignored `perturbation.local.yaml`。**六扰动类型**：格式类 `drop_suffix`/`width_variant`(ASCII 转全角)/`noise_wrap`(整句包裹)，错字类 `transpose`(相邻字对调)/`homophone`(同音字替换，手工同音表 ~30 字、零依赖、覆盖有限)/`drop_char`(漏字)。**真库结果（3506 家、seed=20260716、129 条）**：overall_recovery=0.78（此值是各类等权平均、意义有限，看分类型）；**格式类全 recovery=1.00**（去后缀片段兜底、整句全名子串、全半角 NFKC 吸收）；**错字类：同音字 0.72 / 漏字 0.60 / 相邻字对调 0.56**——**反直觉发现：单点错（同音/漏字）比对调更易恢复**，因单点只破坏 1 位、词干另一侧 ≥4 字连续片段仍被 `_partial_name_score` 救回；对调扰动 2 相邻位、破坏更狠。**wrong 恒 0**（79→129 条从不误解析别家，失败必为 not_found，符合绝不猜测；对尽调工具 miss 远比 wrong 安全）。width_variant 真库仅 4 例（名含 ASCII 者少）。结论：resolver 对合格/格式类输入够用、错得安全；错字缺口真实但比单看 transpose 的 0.56 温和（单点错约 28–40% miss）；是否加模糊匹配取决于真实用户是否高频打错字（YAGNI 待定）。全确定性、零 LLM、零网络；合成 golden `perturbation.synthetic.yaml` 提交、真库 `.local.yaml` 不出库；默认测试集 308 passed。设计/计划见 `docs/superpowers/specs/2026-07-16-eval-c1-perturbation-c2-hybrid-scope-design.md` 与 `docs/superpowers/plans/2026-07-16-eval-c1-perturbation-robustness.md`。

34. **Eval C2 混合 scope 评测**（新 `eval/scope_judge.py` + `models`/`metrics`/`runner`/CLI 扩展 + `CompanyRepository.iter_business_scopes`，分支 `feature/eval-c2-scope-quality`）：给 scope 语义检索补"词面确定性下界 + DeepSeek 判官补语义命中"双指标。**词面层**（零 LLM、CI 安全、默认跑）：`ScopeQueryCase`（查询集可提交、无企业数据）→ 取 retriever top-k 去重到企业级 → `lexical_precision@k`=top-k 里经营范围**原文字面含能力词**的占比、`lexical_recall@k`（**显式标注词面下界代理**，TP 集常达数百、top-10 召回天然低）、`lexical_tp_count`。**判官层**（标 slow/llm、需 key）：`build_deepseek_scope_judge` 复用 classifier 的 OpenAI client 模式、约束式"是/否"判经营范围是否覆盖能力 → `judged_precision@k`、`noise@k`=1−judged、`semantic_gain@k`=judged−lexical（**语义检索净价值**：真相关但不含字面词的命中）。**降级**：无 `DEEPSEEK_API_KEY`/缺 openai → `build_deepseek_scope_judge` 返 None → 跳过判官层、只出词面、不报错。独立子命令 `eval scope-quality --judge`（**v1 `eval scope` recall@k 一字不动**，两套 case schema 不撞）。**真库结果（15 能力词）**：`lexical_precision@k=0.97`、`judged_precision@k=0.99`、`noise@k=0.01`、`semantic_gain@k=0.02`——高频能力词（注塑/模具/五金…）经营范围基本原文写出、词面已近天花板，语义增量小；语义价值预期在"能力被描述但不点名"的查询更大。**判官数据外发**（经营范围原文发 DeepSeek）归用户已授权云端豁免（与记忆线、网页呈现层同级），CLAUDE.md 红线文本不删仅本线豁免。CI 零网络（fake client/fake retriever），真链路标手验。**环境坑（systematic-debugging 修复）**：首跑判官 `judged_precision=0.00` 全 0，根因是 openai SDK 依赖的 `jiter` 原生扩展（cp312）经 `.pth` 桥泄漏进 3.11 conda-env、`ModuleNotFoundError: jiter.jiter`，被判官 `except Exception: return False` 静默吞成假"不覆盖"；装 `jiter==0.16.0`(cp311, `--ignore-installed`) 修复（同 project-memory 环境注意的既有办法），**并去掉判官异常吞咽**——系统性失败改响亮报错，避免把 infra 故障伪装成合法指标。默认测试集 318 passed。设计/计划见 `docs/superpowers/plans/2026-07-16-eval-c2-hybrid-scope.md`。
