...
- **Web 聊天界面**（对外演示 Demo）：`GET /` 返 `web/index.html`、`/static` 挂 `StaticFiles` 托管 `web/{index.html,style.css,app.js,graph.js}`；自包含 vanilla 页（零构建零 npm、无 CDN），发 `POST /session/turn` 后以「研究中…」加载态→结构化报告卡渲染 `SupplierReport`（recommendation 徽章四值映射、证据表带 `local://` 引用、待解问题=尚未接入数据源）；身份 `user_id` 存 localStorage、`session_id` 内存复用多轮指代。后端仅新增静态托管（`WEB_DIR=Path(__file__).parent/"web"`），端点逻辑不变。
- **会话历史删除**：`DELETE /sessions/{session_id}?user_id=...` 由 `JsonSessionStore.delete` 先校验 session_id 格式与会话归属，再删除对应 JSON；不存在或非归属用户均返回 404，非法 ID 返回 400。网页侧栏的删除按钮会二次确认，成功后同步删除该用户 localStorage 中的消息转录；若删除当前会话则回到新对话状态。
- **网页流式呈现（DeepSeek）**：`POST /session/turn/stream`（SSE）建图时注入 scope/graph 检索器，呈现层走 `llm/deepseek.py::build_deepseek_polisher`（`stream=True` 逐 token）。三种报告经 `_resolve_report` 定稿后交 LLM 生成中文；`recommendation` 仍保留在结构化报告中，但不再作为每轮对话的固定「结论」文案输出。呈现器仅复述报告事实、数据缺口和线索级提示，不推断、不改写实体原文；无 `DEEPSEEK_API_KEY`/LLM 异常时回退 `_report_message_chunks`，同样不附加固定结论。Neo4j 不可用时由图层降级 scope。`create_app(polisher="__default__")` 哨兵可注入。数据越境经用户明确豁免（呈现层，与记忆层同级）。
- **网页图谱子图可视化（问题聚焦）**：graph 模式轮次在 `report_start` 前推送 SSE 事件 `graph_subgraph`（`project_subgraph` 把 `HybridContext` 纯函数投影为问题聚焦的 `GraphSubgraph`：查询概念节点 → 命中种子企业（`semantic_match` 边）+ 仅共享控制人（`control_clue` 边）；单一控制人/直接股东/对外投资视为噪声不进载荷，种子为空则子图为空）；前端 `web/graph.js` 在右侧可收起面板用确定性分层布局渲染（查询→种子→共享控制人三层、列数自适应、手写 SVG 零第三方依赖），支持缩放/平移/悬停/点击高亮，窄屏抽屉在图谱到达时自动弹出；红色仅用于 `concentrated_industries` 非空的围标线索，全程「线索级证据 · 须人工复核」口径。named/scope/降级轮次不发事件，面板保持原状；子图不持久化，仅反映本页面会话最新一次图检索。
