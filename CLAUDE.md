...
- `memory/` 是记忆层（`session.py` 确定性会话缓冲+多轮指代 / `service.py` MemoryService 门面+Fake 后端 / `config.py` provider 抽象）。两层：会话最近实体缓冲（指代 `它/该公司/上述`→最近实体，零 LLM）+ mem0 语义记忆（跨会话，云端 DeepSeek 抽取+本地 bge 嵌入+本地 Chroma）。经 `run_research(session=, enable_memory=)` 与 `cli chat` 接入，默认关、API 不动。**红线豁免**：本线经用户明确决定走云端 DeepSeek（`MemoryConfig.deepseek`），本地 Ollama 接口保留（`MemoryConfig.ollama`，一段 config 可切回）；核心红线文本仍在、仅本线豁免，见 `docs/superpowers/specs/2026-07-12-memory-layer-mem0-design.md`。CI 零网络（FakeMemoryBackend），真链路标 `@pytest.mark.llm`。
