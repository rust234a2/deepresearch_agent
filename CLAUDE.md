...
- 评测/可观测（均本地、无外部 LLM、无 LLM-as-judge）：`eval/` 包（企业识别 P/R + scope recall@k，CLI `eval entity|scope`，合成 golden 提交 + 真实 `*.local.yaml` 不出库）；`observability.py` Phoenix 本地追踪（手动 span 在图层包四节点 + root span，`run_research(enable_tracing=)`/CLI `--trace`，`.[trace]` extra，默认关、仅本地）。不引入 RAGAS/LlamaIndex/Phoenix-LLM-eval（撞本地化红线）。
